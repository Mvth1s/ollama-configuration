...
- **Distro detection**: `detect_distro` sets `DISTRO_FAMILY` (arch / debian / fedora / opensuse / unknown)
...
| Script | Role |
|--------|------|
| `01-install-ollama.sh` | Installs Ollama via the official curl script; enables and waits for the systemd service |
| `02-configure-gpu.sh` | Detects GPU vendor, installs drivers/Vulkan packages, writes a systemd drop-in (`/etc/systemd/system/ollama.service.d/override.conf`) for AMD Vulkan workarounds or Intel Vulkan; Nvidia uses CUDA with no override needed |
...
| `04-install-webui.sh` | Installs Open WebUI via `pipx` (fallback: `pip`), creates a user-level systemd service on port 8080 |
...
Tiers are defined as associative arrays `MODEL_XS/S/M/L` in `03-pull-models.sh`. Auto-selection is RAM-based (≤8 GB → XS, ≤16 GB → S, ≤32 GB → M, >32 GB → L), with a forced downgrade to S on CPU-only machines regardless of RAM.

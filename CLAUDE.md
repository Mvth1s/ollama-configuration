...
The `#download` section (and the hero's "Télécharger les apps de bureau" button, which anchors there instead of linking out to the generic Releases page) renders one card per app (`gui`/`launcher`), each with a Linux row (`.deb`/`.rpm`/`.AppImage`) and a Windows row (`.msi`/`.exe`). A small inline `<script>` fetches `api.github.com/repos/.../releases/latest` client-side and matches each asset by `name.startsWith(<tauri productName, spaces→dots>)` + file extension — deliberately **not** a hardcoded `/releases/latest/download/<exact filename>` link per format, because the filename's version comes from `tauri.conf.json` (`0.1.0` at the time of writing), which — per the note on `tauri.conf.json`'s `version` field above — is **not** kept in sync with the release tag; a hardcoded filename would silently 404 the day that version is bumped without updating this file. The dynamic fetch sidesteps that entirely, at the cost of a runtime dependency on GitHub's API (only third-party network call this otherwise fully static page makes) — a failed/rate-limited fetch falls back to a plain link to the Releases page (`#dl-fallback`). Windows assets don't exist yet in any published release (`build-desktop.yml`'s Windows job failed on `v1.0.1` — see the icon bit-depth note above — and no release has been re-tagged since the fix), so those buttons currently render as a disabled "bientôt" (soon) state; this is expected to resolve itself the moment a release actually carries `.msi`/`.exe` assets, with no code change needed. All outbound links on this page (nav, hero, footer, and the per-format download buttons) use `target="_blank" rel="noopener noreferrer"`; in-page anchors (`#screenshots`, `#features`, `#download`, `#install`) do not.
